"""Shared helpers for taoteching-farsi checkers and tools.

This module provides common parsing and utilities needed by the translation
checkers and apparatus:
  * YAML front matter parsing and preservation
  * Blank-line-delimited block iteration (aware of fenced code)
  * Protected span scanner (fences, code spans, comments, tags, links, math)
  * Directive parser (<!-- name: value -->)
  * TOML config reader for pyproject.toml
...
# Spans no normalization rule may touch.
...
def config(section: str, path: Path | None = None) -> dict:
    """The [tool.<section>] table from pyproject.toml, or {} if absent."""
